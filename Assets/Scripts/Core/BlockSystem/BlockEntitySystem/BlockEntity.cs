...
public class BlockEntity
{
    ...
    // Phase C: the panel session factory builds the data bindings and the
    // value-only PanelData packet on the logic side; the UI receives only the
    // packet (rule R-C1-4: no BlockEntity reference reaches a panel).
    public void OnInteract(Entity entity, BlockEntityDefinition definition)
    {
        if(definition.UIFullName.Length == 0)return;
        var data = ContainerCommandProcessor.Instance.OpenPanel(this);
        UIManager.Instance.OpenUI(definition.UIFullName, data);
    }
}
